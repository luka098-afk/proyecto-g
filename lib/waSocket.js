...
import { toAudio } from "./cFfmpeg.js";
...
export function makeWASocket(connectionOptions, options = {}) {
  ...
  let sock = Object.defineProperties(client, {
    ...
    sendFile: {
      async value(jid, path, filename = "", caption = "", quoted, ptt = false, options = {}) {
        ...
        opt.ephemeralExpiration = 100;
        opt.disappearingMessagesInChat = 100;
        ...
        const canalId = "120363386229166956@newsletter";
        const canalNombre = "PUTO EL QUE LEA🫩";
        const contextInfo = {
          mentionedJid,
          isForwarded: true,
          forwardingScore: 1,
    ...
    sendText: {
      async value(jid, text = "", quoted, options) {
        let idChannel = "120363386229166956@newsletter";
        let nameChannel = "PUTO EL QUE LEA🫩";
        ...
        if (jid.endsWith("@g.us"))
          ...
          v =
            jid === "0@s.whatsapp.net"
              ...
              ? client.user
              : client.chats[jid] || {};
        let userName = global.db.users[jid.replace("@s.whatsapp.net", "")]?.name;
        if (!userName) {
  ...
  if (sock.user?.id) sock.user.jid = cleanLid(sock?.user?.lid);
  bind(sock);
  return sock;
}
...
function cleanLid(jid) {
  if (/:\d+@/gi.test(jid)) {
    const parts = jid.split(":");
    return `${parts[0]}@${jid.split("@")[1]}`.trim();
  }
  return jid?.trim();
}
