...
export default {
  ...
  run: async ({ conn, m, remoteJid, isGroup, isAdmin, senderJid }) => {
    try {
      ...
      // Toggle antieliminar
      const estadoActual = global.db.data.chats[remoteJid].antieliminar
      const nuevoEstado = !estadoActual

      global.db.data.chats[remoteJid].antieliminar = nuevoEstado
      ...
      console.log(`${nuevoEstado ? '✅ Antieliminar ACTIVADO' : '❌ Antieliminar DESACTIVADO'}: ${remoteJid}`)

    } catch (err) {
      console.error(`❌ Error en antieliminar.js:`, err.message)
