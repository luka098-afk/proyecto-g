const REGEX = /telegram\.com|t\.me/i;

export default {
  command: ["antitelegram"],
  ...
  run: async ({ conn, m, remoteJid, isGroup, isAdmin, isOwner }) => {
    ...
    // Toggle antiTelegram
    chat.antiTelegram = !chat.antiTelegram;

    // Guardar en DB
    if (!global.db.data.chats[remoteJid]) {
      global.db.data.chats[remoteJid] = {};
    }
    global.db.data.chats[remoteJid].antiTelegram = chat.antiTelegram;

    await conn.sendText(
      remoteJid,
      `✅ *Anti-Telegram ${chat.antiTelegram ? 'activado' : 'desactivado'}*\n\n${
        chat.antiTelegram 
          ? '🚫 Los links de Telegram serán eliminados automáticamente.' 
          : '✅ Los links de Telegram están permitidos.'
      }`,
      m
    );
  },
  ...
  before: async (ctx) => {
    ...
    // Si antiTelegram no está activo, salir
    if (!chat.antiTelegram) return;

    // Detectar link de Telegram
