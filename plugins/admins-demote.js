...
export default {
  ...
  run: async ({ conn, m, args, text, remoteJid, senderJid, isGroup, isAdmin, participants }) => {
    try {
      ...
      // ══════════════════════════════════════════════════════
      try {
        await conn.groupParticipantsUpdate(remoteJid, [realParticipantJid], "demote")

        // ══════════════════════════════════════════════════════
        // 📨 MENSAJE DE CONFIRMACIÓN
        // ══════════════════════════════════════════════════════
        let mensaje = `🔽 *REMOVIDO DE ADMIN* 🔽\n\n`
        mensaje += `━━━━━━━━━━━━━━━━━\n`
        mensaje += `👤 Usuario: @${targetNum}\n`
        mensaje += `❌ Estado: Miembro\n`
        mensaje += `━━━━━━━━━━━━━━━━━\n\n`
        mensaje += `Se ha removido el rango de administrador.`
        ...
        console.log(`✅ ${targetNum} removido de admin en ${remoteJid}`)

      } catch (err) {
        console.error(`❌ Error removiendo admin: ${err.message}`)
        console.log(`⚠️ No se pudo remover admin a ${targetNum}`)

        return await conn.sendText(
          remoteJid,
          `⚠️ No se pudo quitar admin.\n\n` +
      ...
      console.error(`❌ Error en demote.js:`, err.message)
